...
def normalize_values(points,disp):
    ...
    if len(points) < 2 or np.max(points) == np.min(points):
        # Return default values when we don't have enough variation in data
    ...
    if stress_value>=0.75:  # Changed from 75 to 0.75 since we're dealing with probabilities
...
while(True):
    ...
    for detection in detections:
        ...
        # Handle potential NaN values before displaying
        if np.isnan(stress_value):
            display_value = 0
        else:
            display_value = int(stress_value*100)
            
        cv2.putText(frame,"stress level:{}".format(str(display_value)),(20,40),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        cv2.putText(frame,"status: {}".format(stress_label),(20,60),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
